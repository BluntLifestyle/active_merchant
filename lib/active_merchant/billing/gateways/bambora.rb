require 'active_merchant/billing/gateways/bambora/bambora_core'

module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    # This class implements the Canadian {Bambora}[http://www.bambora.com] payment gateway.
    # It is also named TD Canada Trust Online Mart payment gateway.
    # To learn more about the specification of Bambora gateway, please read the OM_Direct_Interface_API.pdf,
    # which you can get from your Bambora account or get from me by email.
    #
    # == Supported transaction types by Bambora:
    # * +P+ - Purchase
    # * +PA+ - Pre Authorization
    # * +PAC+ - Pre Authorization Completion
    #
    # == Secure Payment Profiles:
    # Bambora supports payment profiles (vaults). This allows you to store cc information with Bambora and process subsequent transactions with a customer id.
    ...
    # To learn more about storing credit cards with the Bambora gateway, documentation can be found at http://developer.bambora.com/documentation/classic-apis
    #
    # To store a credit card using Bambora's Legato Javascript Library (http://developer.bambora.com/documentation/legato) you must pass the singleUseToken in
    ...
    # * login is the Bambora merchant ID, username and password should be enabled in your Bambora account and passed in using the <tt>:user</tt> and <tt>:password</tt> options.
    # * Test your app with your true merchant id and test credit card information provided in the api pdf document.
    # * Bambora does not allow Payment Profiles to be deleted with their API. The accounts are 'closed', but have to be deleted manually.
    #
    #  Example authorization (Bambora PA transaction type):
    #
    #   twenty = 2000
