...
# This test suite assumes that you have enabled username/password transaction validation in your Bambora account.
# You will experience some test failures if username/password validation transaction validation is not enabled.
# Bambora does not allow Payment Profiles to be deleted with their API. The accounts are 'closed', but have to be deleted manually.  Because of this, some of these tests will
...
class RemoteBamboraTest < Test::Unit::TestCase
  ...
  def setup
    ...
    # Bambora test cards. Cards require a CVV of 123, which is the default of the credit card helper
  ...
  def generate_single_use_token(credit_card)
    uri = URI.parse('https://www.bambora.com/scripts/tokenization/tokens')
